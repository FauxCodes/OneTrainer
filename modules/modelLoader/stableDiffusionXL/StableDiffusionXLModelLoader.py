...
class StableDiffusionXLModelLoader(
    ...
    def _default_sd_config_name(
        ...
        match model_type:
            ...
            case ModelType.STABLE_DIFFUSION_XL_10_BASE_FLOW:
                return "resources/model_config/stable_diffusion_xl/sd_xl_base.yaml"
            case ModelType.STABLE_DIFFUSION_XL_10_BASE_FLOW_FLUX2_VAE:
                return "resources/model_config/stable_diffusion_xl/sd_xl_base_flux2_vae.yaml"
            case _:
                return None
    ...
    def __load_safetensors_flux2_vae(
            self,
            model: StableDiffusionXLModel,
            model_type: ModelType,
            weight_dtypes: ModelWeightDtypes,
        ...
        pipeline_vae_sd = pipeline.vae.state_dict()

        bn_sd = {
            k[len("first_stage_model."):]: v
            for k, v in load_file(base_model_name).items()
            if k.startswith("first_stage_model.bn.")
        }
        pipeline_vae_sd.update(bn_sd)
        del bn_sd

        flux2_vae = AutoencoderKLFlux2()
        flux2_vae.load_state_dict(pipeline_vae_sd)
        del pipeline_vae_sd
        ...
        if vae_model_name:
            ...
            vae = self._convert_diffusers_sub_module_to_dtype(
                flux2_vae, weight_dtypes.vae, weight_dtypes.fallback_train_dtype
            )
    ...
    def __load_safetensors(
        ...
        if model_type == ModelType.STABLE_DIFFUSION_XL_10_BASE_FLOW_FLUX2_VAE:
            self.__load_safetensors_flux2_vae(model, model_type, weight_dtypes, base_model_name, vae_model_name, quantization)
            return
